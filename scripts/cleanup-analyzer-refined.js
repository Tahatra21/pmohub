...
/**
 * PMO Code Cleanup Analysis - Refined Version
 * Focuses on actual code files and excludes configuration files
 */
...
class RefinedCleanupAnalyzer {
  ...
  async analyze() {
    console.log('🔍 Starting Refined PMO Code Cleanup Analysis...');
    ...
    console.log('✅ Refined analysis complete! Check reports/ directory for results.');
  }

  buildFileInventory() {
    console.log('📁 Building refined file inventory...');
  ...
  shouldSkipFile(filename, fullPath) {
    ...
    const skipDirs = ['node_modules', '.next', 'dist', 'coverage', '.git', 'backups'];
  ...
  detectDuplicates() {
    console.log('🔍 Detecting duplicate code files...');
  ...
  detectUnusedFiles() {
    console.log('🗑️ Detecting unused code files...');
    ...
    console.log(`🗑️ Found ${this.unusedFiles.length} potentially unused code files`);
  }
  ...
  async generateReports() {
    console.log('📊 Generating refined reports...');
    ...
    fs.writeFileSync('reports/duplicates_refined.json', JSON.stringify({
      summary: {
    ...
    fs.writeFileSync('reports/unused_files_refined.json', JSON.stringify({
      summary: {
    ...
    console.log('📊 Refined reports generated in reports/ directory');
  }

  generateRemovalPlan() {
    const plan = `# PMO Code Cleanup Removal Plan - Refined
...
`;
    ...
    fs.writeFileSync('reports/removal_plan_refined.md', plan);
  }
}

// Run the analysis
if (require.main === module) {
  const analyzer = new RefinedCleanupAnalyzer();
  analyzer.analyze().catch(console.error);
}

module.exports = RefinedCleanupAnalyzer;
